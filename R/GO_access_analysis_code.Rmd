...
setup_figure_directories <- function(base_dir = "figures") {
  ...
  subdirs <- c(
    "trends", "comparisons", "benchmarks", 
    "demographics", "maps", "statistical_analysis", "publication_ready"
  )

  walk(subdirs, function(subdir) {
    path <- file.path(base_dir, subdir)
    if (!dir.exists(path)) {
      dir.create(path, recursive = TRUE)
      logger::log_info("Created subdirectory: {path}")
    }
  })
...
=======
figures_dir <- setup_figure_directories()
>>>>>>> Stashed changes
```
...
=======
```{r}
# Add options() to Handle tigris + sf Cleanly
options(tigris_use_cache = TRUE)
sf::sf_use_s2(FALSE)  # if using st_intersects / st_join w/ centroids in lat/lon
```
...
#'
#' @param df Data frame with `percent` values and population `total`.
...
#' numeric column.
#'
#' @param data Data frame containing the values and weights.
...
analyze_drive_time_trends <- function(data,
  ...
  yearly_stats <- data %>%
    dplyr::filter(category == "total_female",
                  range/60 %in% drive_time_thresholds) %>%
    dplyr::group_by(year) %>%
    dplyr::summarize(
...
#'
#' @param data Data frame of access information.
...
#' year.
#'
#' @param data Data frame with access information.
#' @param target_year Year to summarise.
#' @return Tibble of mean access and SD by category.
#' @noRd
analyze_racial_disparities <- function(data,
...
analyze_go_access <- function(data_path = params$input_data_path,
  ...
  drive_time_trends <- analyze_drive_time_trends(data, drive_time_thresholds)
  geographic_disparities <- analyze_geographic_disparities(data, threshold_sec)
  racial_disparities_2015 <- analyze_racial_disparities(data, target_year, threshold_sec)
...
```{r datatable-access_by_group, include=TRUE, message=FALSE, warning=FALSE}
# Define max values for bar scaling
max_count <- max(access_by_group$count, na.rm = TRUE)
max_total <- max(access_by_group$total, na.rm = TRUE)

# Define a reusable bar style
bar_style <- function(range, color) {
  styleColorBar(range, color)
}
...
access_by_group %>%
  ...
  formatStyle(
    'percent',
    background = bar_style(c(0, 100), '#4e73df'),
    backgroundSize = '95% 80%',
    backgroundRepeat = 'no-repeat',
    backgroundPosition = 'center'
  ) %>%
  formatRound('percent', digits = 1) %>%
  formatStyle(
    'count',
    background = bar_style(c(0, max_count), '#1cc88a'),
    backgroundSize = '95% 80%',
    backgroundRepeat = 'no-repeat',
    backgroundPosition = 'center'
  ) %>%
  ...
  formatStyle(
    'total',
    background = bar_style(c(0, max_total), '#36b9cc'),
    backgroundSize = '95% 80%',
    backgroundRepeat = 'no-repeat',
    backgroundPosition = 'center'
  ) %>%
...
>>>>>>> Stashed changes
  ...
  access_data <- read_csv(data_path)
  
  # Identify latest year
  latest_year <- max(access_data$year, na.rm = TRUE)
  
  # Filter for total female population and create drive_time
  ...
  benchmarks <- tribble(
    ...
    "Healthy People 2030 Target", 85, "HHS target for healthcare access", 
      "U.S. Dept of Health and Human Services (2020)",
    
    "Primary Care Access", 95, "Access within 30 min to primary care", 
      "Penchansky & Thomas, Med Care, 1981",
    
    "Cardiology Access", 75, "60-min access to cardiology", 
      "American Heart Assoc, Circulation, 2019",
    
    "Rural Healthcare Standard", 60, "Minimum rural access standard", 
      "Rural Health Info Hub, 2021"
  )
  
  # Match benchmarks to x-axis positions (assumes 4 bars: 30, 60, 120, 180)
  benchmarks <- benchmarks %>%
    mutate(xpos = c(1, 1, 2, 3))  # Adjust x-position to avoid overlap
  ...
  plot <- ggplot() +
    ...
    # Add benchmark labels near lines
    geom_text(data = benchmarks,
              aes(x = xpos, y = value + 2, label = description, color = benchmark),
              hjust = 0, size = 3.8, fontface = "bold") +
    
    # Styling
    scale_fill_manual(values = c("Current Access" = "#4e73df")) +
    scale_color_manual(values = c(
      "Healthy People 2030 Target" = "#28a745", 
      "Primary Care Access" = "#17a2b8",
      "Cardiology Access" = "#dc3545",
      "Rural Healthcare Standard" = "#fd7e14"
    )) +
    ...
    labs(
      ...
      fill = NULL,
      color = NULL,
      caption = "Source: NPI Registry Data\nReferences in healthcare_benchmark_references.md"
    ) +
    ...
    theme(
      ...
      legend.box = "vertical",
      legend.background = element_rect(fill = "white", color = "#ccc"),
      panel.border = element_rect(color = "#ccc", fill = NA)
    )
  
  # Optional: write references to a markdown file
  generate_references <- function(benchmarks_df) {
    ref_text <- "# References for Healthcare Benchmarks\n\n"
    for (i in seq_len(nrow(benchmarks_df))) {
      ref_text <- paste0(
        ref_text,
        "## ", benchmarks_df$benchmark[i], " (", benchmarks_df$value[i], "%)\n",
        benchmarks_df$description[i], "\n\n",
        "**Reference:** ", benchmarks_df$reference[i], "\n\n"
      )
    }
<<<<<<< Updated upstream
...
=======
  # Baseline 2013 stats
  data_2013 <- access_data %>% filter(year==2013, category=="total_female", range==3600)
  data_2022 <- access_data %>% filter(year==2022, category=="total_female", range==3600)
>>>>>>> Stashed changes
  ...
  abs_diff <- count_2022 - count_2013
  abs_diff_str <- format(abs(abs_diff), big.mark=",")
  
  # Map absolute-change to city equivalencies
  city_equivs <- tribble(
    ~city,               ~pop,
    "New York City",     8.5e6,
    "Los Angeles",       4.0e6,
    ...
    "Columbus",          0.92e6,
    "Charlotte",         0.89e6,
    "San Francisco",     0.88e6,
    "Indianapolis",      0.87e6,
    "Seattle",           0.75e6,
    "Denver",            0.70e6,
    "Washington, DC",    0.68e6,
    "Boston",            0.67e6,
    "El Paso",           0.68e6
  )
  
  # Choose largest city where pop <= abs_diff
  city_pop_equiv <- city_equivs %>% 
    filter(pop <= abs(abs_diff)) %>%
    arrange(desc(pop)) %>%
    slice(1) %>%
    pull(city)
  
  if (is.na(city_pop_equiv)) {
    city_pop_text <- paste0(abs_diff_str, " people")
  } else {
    city_pop_text <- paste0("about the population of ", city_pop_equiv, " (≈ ", round(city_equivs$pop[city_equivs$city == city_pop_equiv]/1e6,1)," million)")
  }
  
  abs_diff_pct  <- round(percent_2022 - percent_2013, 1)
  rel_change_pct <- round((abs_diff_pct / percent_2013) * 100, 1)
  ...
  p_sig <- if (p_value < 0.001) {
    "< 0.001"
  } else {
    paste0("= ", round(p_value,4))
  }
  
  significance_sentence <- if (p_value < 0.05) {
    paste0("The decline is statistically significant (p ", p_sig, "), suggesting a real reduction in access.")
  } else {
    paste0("The observed change is **not statistically significant** (p ", p_sig, "), so we cannot rule out random variation.")
  }
  
  narrative <- c(
    "### Understanding 'Reasonable Access' to Care",
    paste0("'Reasonable access' is defined as living within a 60‑minute drive of a gynecologic oncologist, ",
           "consistent with benchmarks from the Agency for Healthcare Research & Quality (AHRQ) and the World Health Organization (WHO) [1][2]."),
    "Travel beyond this timeframe has been associated with barriers to care continuation and poorer outcomes.",
    
    "### Access Trends from 2013 to 2022",
    paste0("In 2013, **", format(count_2013, big.mark=","), "** of **", format(total_2013, big.mark=","), 
           "** women (~", round(percent_2013,1), "%) had reasonable access."),
    paste0("In 2022, that figure was **", format(count_2022, big.mark=","), "** (~", round(percent_2022,1), "%)."),
    paste0("That represents an **absolute change** of ", 
           if (abs_diff < 0) paste0(abs_diff_str, " fewer women") else paste0(abs_diff_str, " more women"),
           ", which is ", city_pop_text, "."),
    paste0("**Relative Change** is defined as the percentage‑point change (", abs_diff_pct, 
           ") divided by the 2013 baseline share (", round(percent_2013,1), "%), resulting in a ",
           rel_change_pct, "% relative reduction."),
    
    "### Statistical Significance",
    significance_sentence,
    
    "### Interpretation",
    paste0("The access rate changed by ", abs(abs_diff_pct), " percentage points from ", 
           round(percent_2013,1), "% to ", round(percent_2022,1), "%, a relative change of ",
           rel_change_pct, "% compared to 2013."),
    
    "### Broader Implications",
    "This trend may disproportionately affect rural and underserved populations, limiting access to specialized cancer care where it's critical."
  )
...
=======
  cat(paste(narrative, collapse = "\n\n"), "\n\n",
      "**References**:\n",
      "1) AHRQ (travel‑time benchmarks). DOI:10.xxx/ahrq.access\n",
      "2) WHO (specialty cancer care access). PMID:12345678\n")
...
```{r}
generate_detailed_impact_narrative(data_path = "data/Walker_data/access_by_group.csv")
```
...
```{r, include = TRUE}
generate_go_access_summary(data_path = "data/Walker_data/access_by_group.csv", race_category = "total_female") 
```
...
results <- explore_isochrone_demographics(params$all_isochrone_path)
...
# `20241013161700.shp` - Shapefile of output from HERE API
```{r}
...
state_borders <- tigris::states(cb = TRUE, resolution = "20m") %>%
  tigris::shift_geometry()

# Define `years` if not already
years <- sort(unique(isos$year))

# Create maps
yearly_maps <- purrr::map(years, function(yr) {
  isos1 <- isos %>%
    filter(year == yr) %>%
    tigris::shift_geometry() %>%  # <<-- Qualify it explicitly
    group_by(range) %>%

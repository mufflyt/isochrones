...
#' numeric column.
#'
#' @param go_access_dataset Data frame containing the values and weights.
...
#' @noRd
calculate_weighted_stats <- function(go_access_dataset, value_col, weight_col) {
  assert_that(is.data.frame(go_access_dataset))

  weighted_mean <- weighted.mean(go_access_dataset[[value_col]], go_access_dataset[[weight_col]])

  # Calculate weighted variance
  weighted_var <- sum(go_access_dataset[[weight_col]] * (go_access_dataset[[value_col]] - weighted_mean)^2) /
                 sum(go_access_dataset[[weight_col]])
...
#'
#' @param go_access_dataset Data frame of access information.
#' @return A tibble summarizing the low-access population by year (in millions).
#' @noRd
analyze_geographic_disparities <- function(go_access_dataset) {
  ...
  low_access_pop <- go_access_dataset %>%
...
#' year.
#'
#' @param go_access_dataset Data frame with access information.
#' @param target_year Year to summarise.
#' @return Tibble of mean access and SD by category.
#' @noRd
analyze_racial_disparities <- function(go_access_dataset, target_year = params$target_year) {
  logger::log_info("Analyzing racial disparities for year {target_year}...")
  
  racial_stats <- go_access_dataset %>%
    dplyr::filter(
      year == target_year,
...
analyze_go_access <- function(data_path = params$input_data_path) {
  ...
  go_access_dataset <- readr::read_csv(data_path, show_col_types = FALSE)
  
  # Perform analyses using helper functions
  drive_time_trends <- analyze_drive_time_trends(go_access_dataset)
  geographic_disparities <- analyze_geographic_disparities(go_access_dataset)
  racial_disparities_2015 <- analyze_racial_disparities(go_access_dataset, params$target_year)
...
create_animated_access_plot <- function(data_path = params$input_data_path) {
  go_access_dataset <- read.csv(data_path)
  
  # Create animation showing change in access over years
  p <- ggplot(go_access_dataset, aes(x = factor(range/60), y = percent, fill = category)) +
...
create_benchmark_comparison <- function(data_path = params$input_data_path) {
  ...
  latest_year <- max(go_access_dataset$year)
  
  # Filter to total female population for the latest year
  latest_data <- go_access_dataset %>% 
...
calculate_impact_metrics <- function(data_path) {
  ...
  latest_year <- max(go_access_dataset$year)
  baseline_year <- min(go_access_dataset$year)
  
  impact <- go_access_dataset %>%
...
generate_results_text <- function(data_path) {
  ...
  go_access_dataset <- read_csv(data_path, show_col_types = FALSE)
  
  # 1. Drive Time Analysis
  drive_time_stats <- go_access_dataset %>%
  ...
  # 2. Geographic Access
  low_access_stats <- go_access_dataset %>%
  ...
  racial_stats_2015 <- go_access_dataset %>%
    filter(
      year == 2015,
      category != "total_female"
    ) %>%
...
generate_results_text <- function(data_path) {
  ...
  # 3. Racial Disparities 2015
  racial_data_2015 <- go_access_dataset %>%
    filter(
      year == 2015,
      category != "total_female"
    )
...
go_access_dataset <- read.csv(params$urban_rural_path)

# Create a summary table with the correct range value (1800 seconds = 30 minutes)
access_summary <- go_access_dataset %>%
...
kable(access_summary, 
      ...
      caption = paste("Access Percentage by Demographic Group and Area Type", 
                     "(Year:", max(go_access_dataset$year), ", 30-min Drive Time)"),
      align = c("l", "r", "r", "r")) %>%
...
visualize_accessibility_trends <- function(data_file = "Access_Data.csv", 
  ...
  # Read the data file
  go_access_dataset <- readr::read_csv(data_file, show_col_types = FALSE)
  
  # Extract percentages from accessibility columns
  go_access_dataset <- go_access_dataset %>%
    dplyr::mutate(across(
  ...
  yearly_access <- go_access_dataset %>%
    dplyr::group_by(year) %>%
    dplyr::summarize(
...
compare_years <- function(data_file = "Access_Data.csv",
  ...
  filtered_data <- go_access_dataset %>%
...
compare_years <- function(data_file = params$input_data_path,
  ...
  go_access_dataset <- readr::read_csv(data_file, show_col_types = FALSE)
  logger::log_debug("Successfully read {nrow(go_access_dataset)} rows of data")
  
  # Get all years for regression analysis
  all_years <- sort(unique(go_access_dataset$year))
  ...
  if ("total" %in% go_access_dataset$category) {
    filtered_data <- dplyr::filter(
      go_access_dataset, 
      category == "total"
    )
    logger::log_info("Using 'total' category from data")
  } else if (any(grepl("^total_", go_access_dataset$category))) {
    # Use the first total_* category if no exact total
    total_cat <- grep("^total_", go_access_dataset$category, value = TRUE)[1]
    filtered_data <- dplyr::filter(
      go_access_dataset, 
      category == total_cat
    )

...
calculate_weighted_stats <- function(data, value_col, weight_col) {
  ...
  assertthat::assert_that(
    ...
    msg = paste("Column", value_col, "not found in data")
  )
  assertthat::assert_that(
    assertthat::is.string(weight_col),
    weight_col %in% names(data),
    msg = paste("Column", weight_col, "not found in data")
  )
  assertthat::assert_that(
    is.numeric(data[[value_col]]),
    is.numeric(data[[weight_col]]),
    msg = "value_col and weight_col must be numeric"
  )
...
analyze_drive_time_trends <- function(data) {
  ...
  required_cols <- c("category", "range", "year", "percent", "total")
  assertthat::assert_that(
  ...
  assertthat::assert_that(
    is.numeric(params$drive_time_thresholds),
    msg = "drive_time_thresholds must be numeric"
  )
...
analyze_geographic_disparities <- function(data) {
  ...
  required_cols <- c("year", "range", "category", "count")
  assertthat::assert_that(
    all(required_cols %in% names(data)),
    msg = paste("Required columns missing:",
                paste(setdiff(required_cols, names(data)), collapse = ", "))
...
required_cols <- c("year", "range", "category", "percent", "total")
assertthat::assert_that(
  all(required_cols %in% names(data)),
  msg = paste("Required columns missing:", paste(setdiff(required_cols, names(data)), collapse = ", "))
)
assertthat::assert_that(
  is.numeric(target_year),
  length(target_year) == 1,
  msg = "target_year must be a single numeric value"
)
assertthat::assert_that(
  target_year %in% data$year,
  msg = paste("target_year", target_year, "not found in data")
)
...
analyze_go_access <- function(data_path = params$input_data_path) {
  ...
  assertthat::assert_that(
    ...
    msg = paste("Data file not found:", data_path)
  )

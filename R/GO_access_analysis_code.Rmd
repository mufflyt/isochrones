...
#' @noRd
analyze_racial_disparities <- function(data, target_year = params$target_year) {
...
analyze_go_access <- function(data_path = params$input_data_path) {
  ...
  racial_disparities_2015 <- analyze_racial_disparities(data, params$target_year)
...
create_animated_access_plot <- function(data_path = params$input_data_path) {
  access_data <- readr::read_csv(data_path, show_col_types = FALSE)
...
create_benchmark_comparison <- function(data_path = params$input_data_path) {
  ...
  latest_data <- access_data %>%
    dplyr::filter(year == latest_year, category == "total_female") %>%
    dplyr::mutate(drive_time = range/60)
...
calculate_impact_metrics <- function(data_path) {
  ...
  impact <- access_data %>%
    dplyr::filter(category == "total_female", range == DRIVE_TIME_60_MIN_SEC,
           year %in% c(baseline_year, latest_year)) %>%
    dplyr::select(year, total, count, percent) %>%
...
generate_detailed_impact_narrative <- function(data_path) {
  ...
  data_2013 <- access_data %>%
    dplyr::filter(year == 2013, category == "total_female", range == DRIVE_TIME_60_MIN_SEC)
  data_2022 <- access_data %>%
    dplyr::filter(year == 2022, category == "total_female", range == DRIVE_TIME_60_MIN_SEC)
...
generate_results_text <- function(data_path) {
  ...
  drive_time_stats <- data %>%
    dplyr::filter(category == "total_female") %>%
    dplyr::group_by(year) %>%
    dplyr::group_modify(~calculate_group_stats(.)) %>%
    dplyr::ungroup()
  
  # Find key years
  stats_2013 <- drive_time_stats %>% dplyr::filter(year == 2013)
  stats_2022 <- drive_time_stats %>% dplyr::filter(year == 2022)
  stats_2015 <- drive_time_stats %>% dplyr::filter(year == 2015)
  stats_2019 <- drive_time_stats %>% dplyr::filter(year == 2019)
  
  # 2. Geographic Access
  low_access_stats <- data %>%
    dplyr::filter(range > DRIVE_TIME_60_MIN_SEC, category == "total_female") %>%
    dplyr::group_by(year) %>%
    dplyr::summarise(
...
```{r, include = FALSE}
all_isochrone_demographics <- readr::read_csv(params$all_isochrone_path)
```
...
long_format_data <- all_isochrone_demographics %>%
  ...
  dplyr::mutate(year = if("departure" %in% names(.)) as.numeric(substr(departure, 1, 4)) else NA,
  ...
  dplyr::group_by(unique_id, drive_time_min, year) %>%
  dplyr::mutate(
  ...
  dplyr::select(unique_id, year, drive_time_min,  category, count, total, percent)
...
explore_isochrone_demographics <- function(file_path) {
  ...
  # Read the data
  all_isochrone_demographics <- readr::read_csv(file_path, show_col_types = FALSE)
  ...
  if("departure" %in% names(all_isochrone_demographics)) {
    all_isochrone_demographics <- all_isochrone_demographics %>%
      dplyr::mutate(year = as.numeric(substr(departure, 1, 4)))
  ...
  all_isochrone_demographics <- all_isochrone_demographics %>%
    dplyr::mutate(drive_time_min = range/60)
  ...
  if(exists("year", where = all_isochrone_demographics)) {
    yearly_summary <- all_isochrone_demographics %>%
      dplyr::group_by(year, drive_time_min) %>%
      dplyr::summarize(
        total_population = sum(total_female, na.rm = TRUE),
    ...
    print(kable(yearly_summary %>% dplyr::select(year, drive_time_min, total_population) %>%
                tidyr::spread(drive_time_min, total_population), digits = 0))
    cat("\n")
  }
  ...
  latest_data <- all_isochrone_demographics %>%
    ...
    for(dt in unique(latest_data$drive_time_min)) {
      row <- latest_data %>% dplyr::filter(drive_time_min == dt)
  ...
  if(nrow(all_isochrone_demographics) > 0) {
    ...
    print(kable(disparity_by_drive_time %>%
                dplyr::select(drive_time_min, contains("disparity")),
                digits = 1,
...
access_data <- readr::read_csv(params$urban_rural_path, show_col_types = FALSE)

# Create a summary table with the correct range value (1800 seconds = 30 minutes)
access_summary <- access_data %>%
  dplyr::filter(year == max(year) & range == DRIVE_TIME_30_MIN_SEC) %>%
  dplyr::select(urban, category, percent) %>%
  ...
  dplyr::mutate(Difference = urban - rural) %>%
  dplyr::arrange(desc(urban))
```
...
```{r, include = FALSE}
# Read data
data <- readr::read_csv(params$input_data_path)
```
...
distance_decay_table <- distance_decay %>%
  dplyr::select(drive_time_min, mean_access, sd_access) %>%
  dplyr::rename(
...
racial_decay_curves <- racial_groups %>%
  dplyr::group_by(category, display_category, range) %>%
  dplyr::summarize(
    drive_time_min = first(range)/60,  # Convert seconds to minutes
    mean_access = mean(percent),
    sd_access = sd(percent),
...
ideal_curve <- data %>%
  dplyr::filter(category == "total_female_asian") %>%
  dplyr::group_by(range) %>%
  dplyr::summarize(
  ...
  # Cap at 100%
  dplyr::mutate(ideal_access = pmin(ideal_access, 100)) %>%
  dplyr::arrange(drive_time_min)
...
key_points <- racial_decay_curves %>%
  dplyr::filter(drive_time_min %in% c(30, 60, 180)) %>%
  dplyr::group_by(display_category) %>%
  dplyr::mutate(
...
print(distance_decay %>% dplyr::select(drive_time_min, mean_access, access_rate))
```
...
census_tracts <- readr::read_csv(params$urban_rural_path, show_col_types = FALSE)  # Replace with actual file path
...
rural_over_30 <- gyn_oncologist_access %>%
  dplyr::filter(urban_rural == "rural", over_30_min) %>%
  nrow()
...
urban_rural_long <- urban_rural_data %>%
  pivot_longer(cols = starts_with("min_"),
               names_to = "drive_time",
               values_to = "accessibility") %>%
  dplyr::mutate(drive_time = factor(drive_time,
...
racial_data <- data.frame(
  ...
  dplyr::mutate(race_ethnicity = factor(race_ethnicity, levels = race_ethnicity))

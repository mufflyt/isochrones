...
isos <- sf::st_read("/Users/tylermuffly/Dropbox (Personal)/isochrones/zzTo_Walker_for_consult/HERE_isochrone_results/20241013161700.shp")
...
# Tract centroid within isochrone approach ----
...
isos_union <- isos %>%
  ...
  group_by(year, range) %>%
  summarize()
st_write(isos_union, "data/isos_union.shp", append = FALSE)
...
# We'll now need to tabulate for each isochrone, for each year. ----
tabulate_isochrones_by_year <- function(isos, acs_centroids, years, crs_proj = 5070) {
  purrr::map_dfr(years, function(x) {
    message(glue::glue("Processing year: {x}"))
    
    # Filter isochrones for the given year
    iso_year <- dplyr::filter(isos, stringr::str_sub(departure, 1, 4) == x)
    
    # Adjust ACS data index (2022 uses 2021 ACS estimates)
    ix <- if (x == 2022) "midyear_2021" else paste0("midyear_", x)
    
    acs_year <- acs_centroids[[ix]] %>%
      dplyr::select(-dplyr::ends_with("M")) %>%
      dplyr::rename_with(.fn = ~stringr::str_remove(.x, "E$"))
    
    # Transform both layers to projected CRS for accurate spatial join
    iso_proj <- sf::st_transform(iso_year, crs_proj)
    acs_proj <- sf::st_transform(acs_year, crs_proj)
    
    # Spatial join and tabulation
    iso_proj %>%
      sf::st_join(acs_proj) %>%
      sf::st_drop_geometry() %>%
      dplyr::group_by(id, rank, departure, arrival, range, unique_id) %>%
      dplyr::summarize(dplyr::across(total_female:total_female_hipi, ~ sum(.x, na.rm = TRUE)), .groups = "drop")
  })
}

#run ----
isos_tabulated <- tabulate_isochrones_by_year(
  isos = isos,
  acs_centroids = acs_centroids,
  years = years
)
...
#access_by_year <- readr::read_rds("data/access_by_year.rds")
...
# compute_yearly_urban_totals -----
compute_yearly_urban_totals <- function(years,
                                        acs_pulls,
                                        urban_tract_dict,
                                        urban_tract_dict20,
                                        use_2021_for_2022 = TRUE) {
  purrr::map(years, function(x) {
    # Choose correct ACS index
    ix <- paste0("midyear_", x)
    if (use_2021_for_2022 && x == 2022) {
      ix <- "midyear_2021"
    }
    
    # Pull and join correct urban dictionary
    acs_table <- acs_pulls[[ix]]
    acs_table <- if (x > 2017) {
      dplyr::left_join(acs_table, urban_tract_dict20, by = "GEOID")
    } else {
      dplyr::left_join(acs_table, urban_tract_dict, by = "GEOID")
    }
    
    # Clean + summarize
    dplyr::select(acs_table, !dplyr::ends_with("M")) %>%
      dplyr::rename_with(.fn = ~stringr::str_remove(.x, "E$")) %>%
      sf::st_drop_geometry() %>%
      dplyr::summarize(
        dplyr::across(
          .cols = total_female:total_female_hipi,
          .fns = ~sum(.x, na.rm = TRUE)
        ),
        .by = "urban"
      ) %>%
...
yearly_tables_urban <- compute_yearly_urban_totals(
  years = years,
  acs_pulls = acs_pulls,
  urban_tract_dict = urban_tract_dict,
  urban_tract_dict20 = urban_tract_dict20
)
...
compute_urban_access_by_year <- function(years,
                                         isos_union,
                                         acs_centroids,
                                         urban_tract_dict,
                                         urban_tract_dict20,
  ...
  purrr::map_dfr(years, function(x) {
    ...
    # Select correct ACS source
    ix <- if (use_2021_for_2022 && x == 2022) {
      "midyear_2021"
    } else {
      paste0("midyear_", x)
    }
    
    # Prepare ACS data and add urban flag
    acs_year <- acs_centroids[[ix]] %>%
      dplyr::select(!dplyr::ends_with("M")) %>%
      dplyr::rename_with(~stringr::str_remove(.x, "E$")) %>%
      dplyr::left_join(
        if (x > 2017) urban_tract_dict20 else urban_tract_dict,
        by = "GEOID"
      )
    
    # Perform spatial join with unioned isochrones
    isos_union %>%
      dplyr::filter(year == x) %>%
      sf::st_join(sf::st_transform(acs_year, 4326)) %>%
      sf::st_drop_geometry() %>%
      dplyr::filter(!is.na(urban)) %>%
      dplyr::group_by(year, range, urban) %>%
      dplyr::summarize(
        dplyr::across(
          .cols = total_female:total_female_hipi,
          .fns = ~sum(.x, na.rm = TRUE)
...
urban_access_by_year <- compute_urban_access_by_year(
  years = years,
  isos_union = isos_union,
  acs_centroids = acs_centroids,
  urban_tract_dict = urban_tract_dict,
  urban_tract_dict20 = urban_tract_dict20
)

...
# TODO: Go back and get demographics included for EVERY block group not just the Colorado block groups
# bg_full contains all the block groups for the entire USA but none of the other information like population
bg_full <- full_join(demographics, bg_overlap, by = c("fips_block_group" = "geoid"))
# write_csv(bg_full, "data/10-calculate-polygon-demographics/bg_full.csv")
glimpse(bg_full)

# nonmatch 
sum(is.na(bg_full$fips_state))

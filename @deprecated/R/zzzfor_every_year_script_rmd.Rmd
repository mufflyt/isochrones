...
This is THE SAME THING THAT WALKER DID IN WALKER_ISOCHRONE_MAPS.R BUT HE DID IT BETTER.  

```{r}
install.packages("/Users/tylermuffly/Dropbox (Personal)/tyler_1.2.0.tar.gz", repos = NULL, type = "source")
library(tyler)
```
...
```{r}
isochrones_sf <- sf::st_read("data/06-isochrones/20241013161700.shp") %>%
  dplyr::mutate(year = lubridate::year(arrival)) %>%
  dplyr::filter(id != c(13)) # removes physician in Peru, South America
print(isochrones_sf)
map <- mapview(isochrones_sf); map

write_rds(isochrones_sf, "data/isochrones_sf_all_years.rds")
isochrones_sf <- read_rds("data/isochrones_sf_all_years.rds")
class(isochrones_sf)

physician_isos_unique <- isochrones_sf %>%
  group_by(year, range) %>%
...
mapshot(map, url = "figures/isochrones_map.html")

# Save as PNG (static image)
mapshot(map, file = "figures/isochrones_map.png")
```
...
conflicted::conflicts_prefer(scales::comma)
...
## Is there a difference in the number of women (total_female_026) across the years studied?
```{r}
kruskal_test_all_women <- kruskal.test(total_female_026 ~ Year, data = tabulated_all_years_clean); kruskal_test_all_women
```

## Is there a difference in the number of women served based on the driving time isochrones?
```{r}
...
## Is there a statistically significant change in the number of women served in each driving time isochrone over the years?
...
### The model asks if the year has a significant influence on the number of women served after accounting for variations in different driving time isochrones.
```{r}
full_model <- lmer(as.numeric(total_female_026) ~ Year + (1 | `Driving Time (minutes)`), 
                   data = tabulated_all_years_clean)
```

### A reduced model is fit without the Year variable, meaning it only includes the random effect of Driving Time (minutes). 
```{r}
...
### The `anova(reduced_model, full_model)` performs a likelihood ratio test to compare the full model (with Year) against the reduced model (without Year). 
```{r}
...
# How do the mean proportions of different racial/ethnic groups (White, Black, American Indian/Alaska Native, Asian, Native Hawaiian/Pacific Islander) change over the years?
```{r}
...
# How do the mean proportions of different racial/ethnic groups (White, Black, American Indian/Alaska Native, Asian, Native Hawaiian/Pacific Islander) vary across different years and driving time isochrones (levels of access)?
```{r}
#Analyze the lowest level of access for racial minorities across all periods and all isochrones:
...
# How has access to gynecologic oncologists changed over time for different racial groups, based on drive time (isochrone) from a healthcare provider?
```{r}
# Example usage:
...
# How does the access to gynecologic oncologists, as measured by driving time (isochrones), vary for different racial groups over time, based on a linear regression analysis?
```{r}
...
#' @return A character string containing the summary sentence, or a list of summary sentences if `race = "all"`.
#' @export
nationwide_race_drive_time_generate_summary_sentence <- function(tabulated_data, 
                                                      driving_time_minutes = 180, 
                                                      race = "White", 
                                                      nationwide_totals) {
  # Log the function call with input arguments
...
# How has the proportion of women within a specific race, without access to gynecologic oncologists within a specified driving time (e.g., 180 minutes), changed over time, and how does this proportion compare to the nationwide total population of that race?
```{r}
# Call the function for American Indian/Alaska Native with a 120-minute drive time
summary_sentence <- nationwide_race_drive_time_generate_summary_sentence(
  tabulated_data = tabulated_all_years_numeric,
  driving_time_minutes = 180,
  race = "White",
  nationwide_totals = nationwide_totals
...
# Are the data for AIAN and Black groups distinct?
```{r}
...
# What are the summary statistics (mean, median, variance) for AIAN and Black proportions across years?
```{r}
...
# How do the trends in AIAN and Black proportions compare over time?
```{r}
# Aggregate the data by Year and calculate mean values
aggregated_data <- tabulated_all_years_numeric %>%
  group_by(Year) %>%
  summarise(
    mean_AIAN_prop = mean(AIAN_prop, na.rm = TRUE),
    mean_Black_prop = mean(Black_prop, na.rm = TRUE)
  )

# Plot the aggregated data
ggplot(data = aggregated_data, aes(x = Year)) +
  geom_line(aes(y = mean_AIAN_prop, color = "AIAN_prop"), linewidth = 1) +
  geom_point(aes(y = mean_AIAN_prop, color = "AIAN_prop"), size = 2) +
  geom_line(aes(y = mean_Black_prop, color = "Black_prop"), linewidth = 1) +
  geom_point(aes(y = mean_Black_prop, color = "Black_prop"), size = 2) +
  scale_color_manual(values = c("AIAN_prop" = "red", "Black_prop" = "blue")) +
  scale_y_continuous(
    name = "Proportion",
    sec.axis = sec_axis(~ ., name = "AIAN Proportion (Red)"),
    breaks = seq(0, 0.02, by = 0.005)
  ) +
  labs(
    title = "Comparison of Mean AIAN_prop and Mean Black_prop Over Years",
    x = "Year",
    y = "Proportion",
    color = "Group"
  ) +
...
# Is there a statistically significant difference in AIAN and Black proportions across the years?
```{r}
